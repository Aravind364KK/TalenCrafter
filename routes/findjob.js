...
router.post("/userhome", async(req,res)=>{
    ...
    if (lowerComname !== "") {
        whereClause += `(LOWER(employerProfile.companyname) LIKE '%' || $${queryParams.length + 1} || '%')`;
        queryParams.push(lowerComname);
    }
    if (lowerLocation !== "") {
        if (whereClause !== '') whereClause += ' AND ';
        whereClause += `(LOWER(jobs.location) LIKE '%' || $${queryParams.length + 1} || '%')`;
        queryParams.push(lowerLocation);
    }
    if (lowerJobtitle !== "") {
        if (whereClause !== '') whereClause += ' AND ';
        whereClause += `(LOWER(jobs.title) LIKE '%' || $${queryParams.length + 1} || '%')`;
...
router.post("/", upload.single("pdfFile"), async (req,res)=>{
    ...
    try{
        ...
        res.render("candidate/findjob.ejs", {userData: data});
    }catch(err){
        console.log(err);
    }
});
