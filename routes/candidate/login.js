...
router.get("/userhome", async(req,res)=>{
    if(req.isAuthenticated()){
        ...
        const employerdet = await db.query(`SELECT * FROM jobs JOIN  employerProfile ON jobs.employer_id = employerProfile.id JOIN employers ON employers.id=employerProfile.id`);
        if(!globalresult){
